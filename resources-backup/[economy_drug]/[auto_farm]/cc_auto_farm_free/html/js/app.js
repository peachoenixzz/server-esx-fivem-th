...
async function chooseAutoItem(currentAutoFarm) {
  $.post(
    "https://cc_auto_farm_free/NUIStartAuto",
        ...
        <div class="bg-black/[0.5] w-[35vh] h-full text-white">
          <div class="flex justify-center">
            <div class="box-content w-[13vh] h-[13vh] mt-[8vh]">
              <img src="img/ui-autofarm-icon.png" />
            </div>
          </div>
  ...
  if (config) {
    config.forEach((item) => {
      itemsHTML += `
          <button 
              class="bg-black/[0.5] hover:bg-gradient-to-l from-[#ffa800]/[0.3] to-[#ffcc00]/[0.3] w-full h-[5vh] rounded-[0.8vh]"
              onclick="chooseAutoItem('${item.name}')"
          >
            ${item.label}
          </button>`;
    });
...
function closeUI() {
  $.post("https://cc_auto_farm_free/NUIFocusOff", JSON.stringify({}));
}
