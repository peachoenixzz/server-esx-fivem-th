...
Citizen.CreateThread(function()
	local expireDate = os.time{year=2024, month=1, day=12}
	print(os.time() >= expireDate)
...
ESX.RegisterServerCallback('carry_cc:checkPlayerAlive', function(source, cb,id)
	...
	if xPlayer then
		cb(xPlayer.getJob().name == "police" or xPlayer.getJob().name == "ambulance")
		return
	end
	cb(true)
end)
