...
$(function() {
  ...
  document.onkeyup = function(e) {
    ...
    if (e.keyCode == 27) { // Escape key
      ...
      $.post('http://minigame_lockpick/escape', JSON.stringify({}));
    }
  };
...
function damagePin() {
  if (!pinLastDamaged || Date.now() - pinLastDamaged > pinDamageInterval) {
    ...
    if (pinHealth <= 0) {
      //$.post('http://minigame_lockpick/removepin', JSON.stringify({}));
      breakPin();
    }
  }
}
...
function reset() {
  var duration = 7 * 1000; // it should finish in 5 seconds !
  ...
  interval = setInterval(function() {
      ...
      // percent = val
      // console.log(percent);
      document.getElementById("myH1").style.width = (100 - val)+'%';
      if (diff >= (7 * 1000)) {
          breakPin()
          clearInterval(interval);
          return
...
function outOfPins() {
  ...
  reset()
  setTimeout(function(){ location.reload(); }, 500);
  $.post('http://minigame_lockpick/process', JSON.stringify({
    state: false
  }));
}
