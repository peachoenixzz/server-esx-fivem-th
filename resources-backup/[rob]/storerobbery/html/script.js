...
Keypad.Open = function(data) {
    ...
    $( "#keypad" ).html(
        "<form action='' method='' name='PINform' id='PINform' autocomplete='off' draggable='true'>" +
            ...
            "<br>" +
            "<input type='button' class='PINbutton clear' name='-' value='CLR' id='-' onClick=clearForm(this); />" +
            "<input type='button' class='PINbutton' name='0' value='0' id='0' onClick=addKeyPadNumber(this); />" +
            "<input type='button' class='PINbutton enter' name='+' value='ENT' id='+' onClick=submitForm(PINbox); />" +
        "</form>"
    );
}
...
$(function () {
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "kekw") {
            $("body").css("background-image", "url('httpss://i.kym-cdn.com/entries/icons/original/000/031/051/cover4.jpg')");
