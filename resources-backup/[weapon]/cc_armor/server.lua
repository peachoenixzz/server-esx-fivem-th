		...
		if xItem.count >= 1 then
			...
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					 ...
					 content = ('%s อาชีพ %s ได้ทำการสวมใส่ %s สูญเสีย %s จำนวน 1 ตัว เหลือ %s ในกระเป๋าจำนวน %s ตัว'):format(xPlayer.name, xPlayer.job.name, xItem.name, xItem.name, xItem.name, xPlayer.getInventoryItem("vest").count),
					 source = xPlayer.source,
				})
			end) -- End log
		end
		...
		if xItem.count >= 1 then
			...
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					 ...
					 content = ('%s อาชีพ %s ได้ทำการสวมใส่ %s สูญเสีย %s จำนวน 1 ใบ เหลือ %s ในกระเป๋าจำนวน %s ใบ'):format(xPlayer.name, xPlayer.job.name, xItem.name, xItem.name, xItem.name, xPlayer.getInventoryItem("helmet").count),
					 source = xPlayer.source,
				})
			end) -- End log
		end
