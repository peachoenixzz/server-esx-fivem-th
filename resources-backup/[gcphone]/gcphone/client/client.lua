...
AddEventHandler("gcPhone:TgiannSes", function(phoneId)
  --print(phoneId)
  --print("getplayer = phone id",GetPlayerServerId(PlayerId()) == tonumber(phoneId))
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    --print("ez call")
    --exports["xsound"]:Cal("iphonex.mp3", true)
    exports.nc_notify:PushNotification({
      scale = 1.0, -- ปรับขนาดของ Notify
      title = 'มีสายเรียกเข้า', -- หัวเรื่อง
      description = 'มีคนกำลังโทรหาคุณ', -- รายละเอียด หากต้องการให้แสดงปุ่มกดให้ใช้ tag <btn></btn>
      type = 'warning',
      position = 'bottom', -- ตำแหน่งการแสดง ('top': บน | 'bottom': ล่าง) * หากไม่ใส่จะเป็น 'top'
      direction = 'right', -- ตำแหน่งการแสดง ('left': ซ้าย | 'center': กลาง' | 'right': ขวา) * หากไม่ใส่จะเป็น 'right'
      priority = 'medium', -- ความสำคัญของ Notify นั้นๆ high จะอยู่บน และ low จะอยู่ล่าง * หากไม่ใส่จะเป็น 'medium'
      icon = 'phonecall', -- Icon (html/img/icons) ที่ต้องการให้แสดง (ใช้ไม่ได้กับ category = 'item')
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
        ...
        exports.nc_notify:PushNotification({
          scale = 1.0, -- ปรับขนาดของ Notify
          title = 'มีข้อความเข้า', -- หัวเรื่อง
          description = text..' ส่งข้อความถึงคุณ', -- รายละเอียด หากต้องการให้แสดงปุ่มกดให้ใช้ tag <btn></btn>
          type = 'warning',
          position = 'bottom', -- ตำแหน่งการแสดง ('top': บน | 'bottom': ล่าง) * หากไม่ใส่จะเป็น 'top'
          direction = 'right', -- ตำแหน่งการแสดง ('left': ซ้าย | 'center': กลาง' | 'right': ขวา) * หากไม่ใส่จะเป็น 'right'
          priority = 'medium', -- ความสำคัญของ Notify นั้นๆ high จะอยู่บน และ low จะอยู่ล่าง * หากไม่ใส่จะเป็น 'medium'
          icon = 'message', -- Icon (html/img/icons) ที่ต้องการให้แสดง (ใช้ไม่ได้กับ category = 'item')
          duration = 3000 -- ระยะเวลาการแสดง Notify
        })
...
function CheckItemPhone (item_name)
...
function TooglePhone() 
  CheckItemPhone(Config.Phones)
    if PhoneWork == Config.Useitem then
...
--[[function TooglePhone() 
      ...
      if menuIsOpen == true then 
        ...
        ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
            if money then
                SendNUIMessage({event = 'updateSocietyBalance', society = money})
            end
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  while takePhoto do
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            ...
            --print(data.field)
            --print(tokenAuth)
              exports['screenshot-basic']:requestScreenshotUpload('https://mongkol.dev/api/img/create', 'file', {
                quality = 0.80,
                encoding = 'jpg',
                headers = {
                  ['Authorization']   = ("Bearer %s"):format(tokenAuth),
              ...
              CellCamActivate(false, false)
                  --print(image)
              cb(json.encode({ url = image.image_url }))
            end)
            takePhoto = false
            --PhonePlayIn()
        end
