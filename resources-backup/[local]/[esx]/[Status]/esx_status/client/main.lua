...
AddEventHandler('esx_status:load', function(status)
	...
	for i=1, #Status, 1 do
		for j=1, #status, 1 do
			if Status[i].name == status[j].name then
				print("esx_status load",Status[i].name)
	...
	CreateThread(function()
		...
		while ESX.PlayerLoaded do
			if exports.cc_savezone:checkInZoneSaveZone1st() then
				--print("goto endTick")
				goto endTick
			end
			if exports.cc_savezone:checkInZoneSaveZone2rd() then
				--print("goto endTick")
				goto endTick
			end
			for i=1, #Status do
