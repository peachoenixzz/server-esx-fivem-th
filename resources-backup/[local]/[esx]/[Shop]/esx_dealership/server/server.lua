            ...
            if stockQtd > 0 then           
                if xPlayer.getAccount('bank').money >= tonumber(vehiclePrice) then
                    ...
                    function (rowsChanged)                     
                        ...
                        local sendToDiscord = '' .. xPlayer.name .. ' ซื้อรถ ' .. name .. ' ทะเบียน ' .. vehicleProps.plate .. ' ราคา $' .. ESX.Math.GroupDigits(vehiclePrice) ..''
                        TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'BuyVehicleDealership', sendToDiscord, xPlayer.source, '^2')
                    end)
                else
