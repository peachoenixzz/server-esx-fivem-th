$('document').ready(function() {
    ...
    MythicProgBar.Progress = function(data) {
        ...
        $("#progress-bar").stop().css({"width": 0, "background-color": "#0059ff"}).animate({
          width: '103%'
        }, {
          ...
          complete: function() {
            ...
            $.post('http://porpy_progressbar/actionFinish', JSON.stringify({
                })
            );
          }
        });
    ...
    MythicProgBar.ProgressCancel = function() {
        ...
        $("#progress-bar").stop().css( {"width": "101%", "background-color": "#ff0000"});

        setTimeout(function () {
            $(".progress-container").css({"display":"none"});
            $("#progress-bar").css("width", 0);
            $.post('http://porpy_progressbar/actionCancel', JSON.stringify({
                })
            );
        }, 1000);
    };
