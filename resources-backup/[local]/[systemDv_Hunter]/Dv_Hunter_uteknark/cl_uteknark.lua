...
local activePlants = {}
local text1 = "~w~ อย่างอื่น ~w~:~r~ ทำลายต้นกัญชา ~w~[~r~Q~w~]"
local text2 = "~g~ ดูแลบำรุง ~w~:~w~ พร้อมเจริญเติบโต"
local text3 = "~b~ ดูแลบำรุง ~w~:~w~ พร้อมเจริญเติบโต [~g~E~w~]"
local text4 = "~b~ การกระทำ ~w~:~w~ เก็บเกี่ยว [~g~E~w~]"
local textIDP = "~y~ หมายเลขของต้น ~w~:~w~ "
local textFis = "~g~ ความโตของต้น ~w~:~g~ พร้อมเก็บเกี่ยว"
...
function serverlog(...)
    TriggerServerEvent('Dv_Hunter_uteknark:log',...)
end

RegisterNetEvent('Dv_Hunter_uteknark:make_toast')
AddEventHandler ('Dv_Hunter_uteknark:make_toast', function(subject,message)
...
RegisterNetEvent('Dv_Hunter_uteknark:do')
AddEventHandler ('Dv_Hunter_uteknark:do', function(scenarioName, location)
...
RegisterNetEvent('Dv_Hunter_uteknark:attempt_plant')
AddEventHandler ('Dv_Hunter_uteknark:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('Dv_Hunter_uteknark:success_plant', location, soil)
        ...
        if #activePlants > 0 then
            ...
            if closestDistance and not IsPedInAnyVehicle(playerPed) then
                ...
                if closestDistance <= 5 then
                    if closestPlant.stage > 5 then 
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,textFis)
                        
                    elseif closestPlant.stage == 5  then
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,"~g~ ความโตของต้น ~w~:~w~ 99%")
                    elseif closestPlant.stage == 4  then
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,"~g~ ความโตของต้น ~w~:~w~ 85%")
                    elseif closestPlant.stage == 3  then
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,"~g~ ความโตของต้น ~w~:~w~ 70%")
                    elseif closestPlant.stage == 2  then
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,"~g~ ความโตของต้น ~w~:~w~ 60%")
                    elseif closestPlant.stage == 1  then
                        Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.5,"~g~ ความโตของต้น ~w~:~w~ 30%")
                    end
                    Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.4,textIDP..closestPlant.id)
                end
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        ...
                        if IsDisabledControlJustPressed(0, 44) then
                            lastAction = now
                            Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.19,text1)
                            Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.3,text2)
                            DeleteObject(closestPlant.object)
                            TriggerServerEvent('Dv_Hunter_uteknark:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                if closestPlant.stage ~= 6 then
                                    Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.3,text3)
                                    Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.19,text1)
                                    if IsControlJustPressed(0, 38) then
                                        lastAction = now
                                        TriggerServerEvent('Dv_Hunter_uteknark:frob', closestPlant.id, myLocation)
                                    end
                                else
                                    Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.3,text4)
                                    Draw3DText(closestPlant.at.x,closestPlant.at.y,closestPlant.at.z+0.19,text1)
                                    if IsControlJustPressed(0, 38) then
                                        lastAction = now
                                        TriggerServerEvent('Dv_Hunter_uteknark:frob', closestPlant.id, myLocation)
...
RegisterNetEvent('Dv_Hunter_uteknark:toggle_debug')
AddEventHandler ('Dv_Hunter_uteknark:toggle_debug', function()
...
RegisterNetEvent('Dv_Hunter_uteknark:pyromaniac')
AddEventHandler ('Dv_Hunter_uteknark:pyromaniac',function(location)
...
RegisterNetEvent('Dv_Hunter_uteknark:groundmat')
AddEventHandler ('Dv_Hunter_uteknark:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('Dv_Hunter_uteknark:test_forest')
AddEventHandler ('Dv_Hunter_uteknark:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('Dv_Hunter_uteknark:test_forest', forest)
end)
