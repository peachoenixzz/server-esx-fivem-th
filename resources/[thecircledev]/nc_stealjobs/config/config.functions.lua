...
Config.ServerNotificationGetRewards = function(xPlayer, itemList)
	...
	for _, item in ipairs(itemList) do
		...
		if item.key == 'box_supstore' then
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					 event = 'StealSuperStore',
					 content = ('%s ได้รับ %s จำนวน %s ชิ้น จากการขโมยคลังสินค้า'):format(xPlayer.name, item.key, item.quantity),
					 source = xPlayer.source,
				 })
			 end) -- End log
		end
		if item.key == 'box_evidence' then
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					 event = 'StealEvidence',
					 content = ('%s ได้รับ %s จำนวน %s ชิ้น จากการขโมยของกลาง'):format(xPlayer.name, item.key, item.quantity),
					 source = xPlayer.source,
				 })
			 end) -- End log
		end
		if item.key == 'box_amp' then
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					 event = 'StealDrugBox',
					 content = ('%s ได้รับ %s จำนวน %s ชิ้น จากการขโมยกล่องยา'):format(xPlayer.name, item.key, item.quantity),
					 source = xPlayer.source,
				 })
			 end) -- End log
		end
...
Config.ClientNotificationError = function(errText, errValue, errValueType)
	...
	if errText == 'source_inventory_full' then
