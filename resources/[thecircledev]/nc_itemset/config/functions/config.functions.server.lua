...
Config.ServerReceivedItems = function(triggeredItem, xPlayer, itemList)
	...
	for _, item in ipairs(itemList) do
		...
		if triggeredItem == 'box_rcar3' and item.type == 'vehicle' then
			MySQL.rawExecute.await('UPDATE `owned_vehicles` SET `expire_date` = SYSDATE() + INTERVAL 3 DAY, `vehiclename` = ? WHERE `owner` = ? AND `plate` = ?', {"box_rcar3" ,xPlayer.identifier, item.plate})
		end
		...
		if triggeredItem == 'box_polbmwm4' and item.type == 'vehicle' then
			MySQL.rawExecute.await('UPDATE `owned_vehicles` SET `job` = "sos" WHERE `owner` = ? AND `plate` = ?', {xPlayer.identifier, item.plate})
		end
		if triggeredItem == 'box_polr34' and item.type == 'vehicle' then
			MySQL.rawExecute.await('UPDATE `owned_vehicles` SET `job` = "sos" WHERE `owner` = ? AND `plate` = ?', {xPlayer.identifier, item.plate})
		end
