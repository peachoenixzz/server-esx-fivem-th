...
Config.ServerWillRobPlayer = function(xRobber, xTarget)
	if xRobber.getInventoryItem('card_nplayer').count > 0 then
		exports.nc_notify:PushNotification(xRobber.source,{
			scale = 1.0, -- ปรับขนาดของ Notify
			title = 'ระบบปล้น', -- หัวเรื่อง
			description = 'คุณมีบัตรไก่ไม่สามารถปล้นได้', -- รายละเอียด หากต้องการให้แสดงปุ่มกดให้ใช้ tag <btn></btn>
			type = 'warning',
	...
	if xTarget.getInventoryItem('card_nplayer').count > 0 then
		exports.nc_notify:PushNotification(xRobber.source,{
			scale = 1.0, -- ปรับขนาดของ Notify
			title = 'ระบบปล้น', -- หัวเรื่อง
			description = 'ผู้เล่นที่คุณกำลังปล้นมีบัตรไก่ไม่สามารถปล้นได้', -- รายละเอียด หากต้องการให้แสดงปุ่มกดให้ใช้ tag <btn></btn>
			type = 'warning',
			position = 'top', -- ตำแหน่งการแสดง ('top': บน | 'bottom': ล่าง) * หากไม่ใส่จะเป็น 'top'
			direction = 'right', -- ตำแหน่งการแสดง ('left': ซ้าย | 'center': กลาง' | 'right': ขวา) * หากไม่ใส่จะเป็น 'right'
			priority = 'medium', -- ความสำคัญของ Notify นั้นๆ high จะอยู่บน และ low จะอยู่ล่าง * หากไม่ใส่จะเป็น 'medium'
