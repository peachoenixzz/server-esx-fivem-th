    ...
    if ids and #ids > 1 then
            for k, id in ipairs(ids) do
                if string.sub(id, 1, string.len("discord:")) == "discord:" then
                    discord = splitData(id)
                    local row = MySQL.single.await('SELECT `steam_id`,`total_point`, DATE_FORMAT(`expire_date`,"%Y-%m-%d") AS expire_date FROM `vip` WHERE `discord_id` = ?', {
                        discord
                    })
                    if not row then return end
                    playerDetail[src] = {}
                    playerDetail[src].point = row.total_point
                    playerDetail[src].expire_date = row.expire_date
                    _Queue.Priority[row.steam_id] = Config.PriorityRanked[tostring(row.total_point)]
                    --Queue:AddPriority()
                end

                --print(id)
                --print(discord,Queue:IsPriority({id}))
...
function splitData(data)
    ...
    for word in string.gmatch(data, "[^:]+") do
        table.insert(splitStr,word)
    end

    for i,word in ipairs(splitStr) do
        if i == 2 then
            return word
        end
    end
...
function Queue:InsertIdentifier()
    local steamid  = false
    local license  = false
    local discord  = false
    ...
    local id = MySQL.insert.await('INSERT IGNORE INTO `vip` (identifier, discord_id, steam_id, expire_date, last_updated, created_date, priority) VALUES (?,?,?,SYSDATE(),SYSDATE(),SYSDATE(),"Citizen")', {
        license, discord, steamid
    })
    local cash = MySQL.insert.await('INSERT IGNORE INTO `cash_point` (discord_id,point,last_updated,created_date) VALUES (?,0,SYSDATE(),SYSDATE())', {
        discord
    })
    local weight = MySQL.insert.await('INSERT IGNORE INTO `TB_PLAYER_WEIGHT` (discord_id) VALUES (?)', {
        discord
    })
end
...
function Queue:CallbackRankedPoint(src)
        local response = MySQL.query.await('SELECT DISTINCT `total_point` FROM `vip` ORDER BY total_point ASC')
        if response then
            for i = 1, #response do
                local row = response[i].total_point
                --print("row" , i)
                --print("all row",#response)
                --print("data in row",row)
                Config.PriorityRanked[tostring(row)] = i
                --print("data in arr",Config.PriorityRanked[4500])
                --print("data in arr",Config.PriorityRanked[0])
                --counter = counter + 1
            end
        end
end
...
local function playerConnect(name, setKickReason, deferrals)
    local src = source
    Queue:InsertIdentifier(src)
    Queue:CallbackRankedPoint(src)
    ...
    --print(type(playerDetail[src].expire_date))
    --print("max plyer ",Queue.MaxPlayers)
    --print("player in sv", GetNumPlayerIndices())
    --print(string.format(Config.Language.pos ,playerDetail[src].point, playerDetail[src].expire_date, pos, Queue:GetSize(),GetNumPlayerIndices() ,Queue.MaxPlayers, (Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"))
    update(string.format(Config.Language.pos ,playerDetail[src].point, playerDetail[src].expire_date, pos, Queue:GetSize(),GetNumPlayerIndices() ,Queue.MaxPlayers,""))
    ...
    while true do
        ...
        local msg = string.format(Config.Language.pos, playerDetail[src].point, playerDetail[src].expire_date, pos, Queue:GetSize(), GetNumPlayerIndices() ,Queue.MaxPlayers,qTime)
        update(msg, data.deferrals)
    end
end
