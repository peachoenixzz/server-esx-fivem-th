...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	local elements = {
		...
		--{icon = "fas fa-car", title = 'เลือกรถออกจากโรงจอด', action = 'garage'},
		--{icon = "fas fa-car", title = TranslateCap('garage_storeitem'), action = 'store_garage'},
		--{icon = "fas fa-car", title = 'เก็บรถเข้าโรงจอด', action = 'store_garage'},
							...
							if elementG.stored  then
								...
								if foundSpawn then
									...
									ESX.Game.SpawnVehicle(elementG.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
										...
										TriggerServerEvent('esx_vehicleshop:setJobVehicleState', elementG.plate, 2)
