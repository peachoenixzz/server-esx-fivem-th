...
AddEventHandler('esx_ambulancejob:clsearch', function(medicId)
  ...
  if isDead then
    ...
    local playersInArea = ESX.Game.GetPlayersInArea(coords, 30.0)
...
function SendDistressSignal()
  ...
  if not exports.cc_training:checkInZoneTraining() then
    ...
    ESX.TriggerServerCallback('esx_ambulancejob:sendDistressToGang', function()
    ...
    if not exports.cc_bodybag:checkStatusBodyBag() then
      pcall(function()-- Start Log
        exports['azael_dc-serverlogs']:insertData({
          event = 'PressGDistressNormal',
          content = ('ผู้เล่นกด G เพื่อแจ้งหน่วยงาน'):format(),
        })
      end)-- End Log
      exports['Porpy_policealert']:alertNet("dead")
    end
    ...
    if exports.cc_bodybag:checkStatusBodyBag() then
      ...
      pcall(function()-- Start Log
        exports['azael_dc-serverlogs']:insertData({
          event = 'PressGDistressBodyBag',
          content = ('ผู้เล่นกด G เพื่อแจ้งหน่วยงาน (โดนห่อศพ)'):format(),
        })
      end)-- End Log

      exports['Porpy_policealert']:alertNet("dead2")
    end
  end
...
function respawnPedToHospital(ped, coords, heading)
  ...
  exports['Porpy_policealert']:alertNet('bleeding')
end
...
function respawnPedToDeadRoom(ped)
  ...
  exports['Porpy_policealert']:alertNet('bleeding_shroud')
end
...
function RemoveItemsAfterRPDeath(choice)
  ...
  if choice == "training" and not exports.cc_training:checkInZoneTraining() then
    local alreadyBodyBag = exports.cc_bodybag:checkStatusBodyBag()
    TriggerServerEvent('esx_ambulancejob:setDeathStatus', false)
    CreateThread(function()
      ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
...
function startDeathScreen(custom)
  ...
  if not exports.cc_training:checkInZoneTraining() then
    if exports.Dv_Hunter_Check:CheckPoliceToScoreboard() == 0 then
      SendNUIMessage({
...
RegisterNetEvent('esx_ambulancejob:PlayerSendMessageToGang')
AddEventHandler('esx_ambulancejob:PlayerSendMessageToGang', function(senderNumber,readerNumber, coords)
  TriggerServerEvent('esx_addons_gcphone:SenderToGang', senderNumber, readerNumber, 'เพื่อนหมดสติ', coords, {
    PlayerCoords = { x = coords.x, y = coords.y, z = coords.z },
  })
end)

RegisterNetEvent('esx_ambulancejob:alertGang')
AddEventHandler('esx_ambulancejob:alertGang', function(coords)
  exports['Porpy_policealert']:alertNetGang('gang_dead', ESX.PlayerData.job.name, coords)
end)
...
RegisterNetEvent('esx_ambulancejob:citizenReviveSuperAed')
AddEventHandler('esx_ambulancejob:citizenReviveSuperAed', function()
  local playerPed = PlayerPedId()
  TriggerServerEvent('esx_ambulancejob:setDeathStatus', false)
  Citizen.CreateThread(function()
    DoScreenFadeOut(800)
