...
local weapons = {
	...
	["WEAPON_REVOLVER_V1"] = "pt_all",
	["WEAPON_REVOLVER_V2"] = "pt_all",
	["WEAPON_REVOLVER_V3"] = "pt_all",
	["WEAPON_KNUCKLEGOLD"] = "pt_all",
	["WEAPON_MACHETTEGOLD"] = "pt_all",
	["WEAPON_GUTKNIFELPGOLD"] = "pt_all",
	["WEAPON_POOLGOLD"] = "pt_all",
}
...
local downgradeWeapon = {
	...
	["WEAPON_REVOLVER_V2"] = "WEAPON_REVOLVER_V1",
	["WEAPON_REVOLVER_V3"] = "WEAPON_REVOLVER_V2",
	...
	["WEAPON_KNUCKLEGOLD"] = "WEAPON_KNUCKLE_5",
	["WEAPON_MACHETTEGOLD"] = "WEAPON_MACHETE_3",
	["WEAPON_GUTKNIFELPGOLD"] = {
		"box_goldknifed",
	},
	["WEAPON_POOLGOLD"] = "WEAPON_POOLCUE_3",
}
...
AddEventHandler('esx_ambulancejob:citizenRevive', function(playerId,itemName)
	...
	if xPlayer and itemName == 'aed' or itemName == 'gov_aed' or itemName == 'r_aed' or itemName == 'aed_super' then
		local xTarget = ESX.GetPlayerFromId(playerId)
		if xTarget then
			if deadPlayers[playerId] then
					if itemName == 'aed_super' then
						xTarget.triggerEvent('esx_ambulancejob:citizenReviveSuperAed')
					else
			...
			if not isMatching then
				for defaultName,downgradeName in pairs(downgradeWeapon) do
					...
					if defaultName == weaponName then
						...
						if defaultName == "WEAPON_REVOLVER_V1" then
							xPlayer.addInventoryItem("gun_pin", 1)
							xPlayer.addInventoryItem("gun_spring", 1)
							xPlayer.addInventoryItem("gun_barrel", 1)
							pcall(function() -- Start Log
						...
						if defaultName == "WEAPON_GUTKNIFELPGOLD" then
							xPlayer.addInventoryItem("box_goldknifed", 1)
							pcall(function() -- Start Log
								exports['azael_dc-serverlogs']:insertData({
									event = 'ReceivedDownGradeWeapon',
									content = ('%s ได้รับ %s จำนวน 1 ชิ้น'):format(xPlayer.name, "box_goldknifed"),
									source = xPlayer.source,
								})
							end) -- End Log
						end	
...
ESX.RegisterServerCallback('esx_ambulancejob:getPlayerName', function(source, cb, target)
...
ESX.RegisterServerCallback('esx_ambulancejob:sendDistressToGang', function(source, cb)
	local mePlayer = source and ESX.GetPlayerFromId(source)
	-- For Log Purpose
	local playerPed = GetPlayerPed(source)
	local playerCoords = GetEntityCoords(playerPed)
	local gpsCount = mePlayer.getInventoryItem("tracker").count

	if gpsCount > 0 then
		--SetEntityCoords(playerped, playerCoords, false, false, false)
		local xPlayers = ESX.GetExtendedPlayers("job", mePlayer.job.name)
		local alreadySend = false
		for _, xPlayer in pairs(xPlayers) do
			TriggerClientEvent('esx_ambulancejob:PlayerSendMessageToGang', xPlayer.source, mePlayer.get("phoneNumber"),xPlayer.get("phoneNumber"), playerCoords)
			if not alreadySend then
				TriggerClientEvent('esx_ambulancejob:alertGang', xPlayer.source, playerCoords)
				alreadySend = true
			end
		end
		cb()
...
-- 	print("test ambulance script")
-- end, true, { help = TranslateCap('revive_help'), validate = true,})
...
ESX.RegisterUsableItem('aed_super', function(source)
	if not playersReviver[source] then
			playersReviver[source] = true
			TriggerClientEvent('esx_ambulancejob:useItemRevive',source, "aed_super")
			Wait(3000)
			playersReviver[source] = nil
	end
end)
