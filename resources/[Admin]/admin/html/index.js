...
$(function() {
    ...
    function renderItems(items) {
        ...
        $.each(items, function(index, item) {
            $("#itemlist").append(`<div id="inventoryitemwrap"><div class = "inventoryitem" data-itemname=${item.name}><div class="img"><img src="nui://nc_inventory/html/img/items/${item.name}.png" width="120px" height="120px" /></div><div class = "name">${item.label}</div></div></div>`);
            let img = new Image;
            img.src = `nui://nc_inventory/html/img/items/${item.name}.png`;
            img.onerror = function() {
                $('.inventoryitem[data-itemname="' + item.name + '"] > .img').html('<i class="fas fa-box centered" style="color:#ffffff;font-size:5vh;"></i>');
            }
        });
    }
    ...
    $("#searchInput").keyup(function() {
        ...
        let filteredItems = itemsFilter.filter(item => item.label.toLowerCase().includes(searchText));
    ...
    window.addEventListener("message", function(t) {
        ...
        if ("ui" === i.type) 1 == i.status ? a(!0) : a(!1);
            ...
            let e = i.itemslist;
            itemsFilter = e
            e.sort((a,b) => a.name - b.name)
            $.each(e, function(e, a) {
                $("#itemlist").append(`<div id="inventoryitemwrap"><div class = "inventoryitem" data-itemname=${a.name}><div class="img"><img src="nui://nc_inventory/html/img/items/${a.name}.png" width="120px" height="120px" /></div><div class = "name">${a.label}</div></div></div>`);
                let t = new Image;
                t.src = `nui://nc_inventory/html/img/items/${a.name}.png`, t.onerror = function() { $('.inventoryitem[data-itemname="' + a.name + '"] > .img').html('<i class="fas fa-box centered" style="color:#ffffff;font-size:5vh;"></i>') }
            }), $("#weaponlist").html("");
            let a = i.weaponlist;
            $.each(a, function(e, a) {
                $("#weaponlist").append(`<div id="inventoryitemwrap"><div class = "inventoryitem" data-weaponname=${a.name}><div class="img"><img src="nui://nc_inventory/html/img/items/${a.name}.png" width="120px" height="120px" /></div><div class = "name">${a.label}</div></div></div>`);
                let t = new Image;
                t.src = `nui://nc_inventory/html/img/items/${a.name}.png`, t.onerror = function() { $('.inventoryitem[data-weaponname="' + a.name + '"] > .img').html(""), $('.inventoryitem[data-weaponname="' + a.name + '"] > .name').addClass("centered") }
            }), $("#jobs").html("");
            let t = i.joblist,
                n = [];
            ...
            let s = i.vehiclelist;
            $.each(s, function(e, a) { $("#vehiclelist").append(`<div id="inventoryitemwrap"><div class = "inventoryitem" data-vehiclename=${a.model}><div class="img"><i class="fas fa-car centered" style="color:#ffffff;font-size:48px;"></i></div><div class = "name">${a.label}</div></div></div>`) })
        } else if ("coords" == i.type) {
            let e = i.coordData;
            $(".coords").attr("coordData", e.x + ", " + e.y + ", " + e.z).html("<b>X: " + e.x.toFixed(2) + " Y: " + e.y.toFixed(2) + " Z: " + e.z.toFixed(2) +  " H: " + i.heading + "</b>")
        }
        ...
        $.post("https://admin/" + a, JSON.stringify({ playerid: id, inputData: e })), i(!0)
    }), $("#confirm").click(function() {
        let e = $("#confirmaction").attr("data"),
            a = $("#confirmaction").attr("action");
        $.post("https://admin/" + a, JSON.stringify({ playerid: id, confirmoutput: e })), i(!0)
        ...
        let e = $(this).data("itemname"),
            a = parseInt($("#qty").val());
        $.post("https://admin/giveitem", JSON.stringify({ playerid: id, name: e, amount: a }))
    }), $("#weapons").on("click", ".inventoryitem", function() {
        let e = $(this).data("weaponname");
        $.post("https://admin/weapon", JSON.stringify({ playerid: id, weapon: e }))
    }), $("#vehicles").on("click", ".inventoryitem", function() {
        let e = $(this).data("vehiclename");
        $.post("https://admin/spawnvehicle", JSON.stringify({ model: e }))
        ...
        switch (s) {
            ...
            case "announce":
                ...
                break;
            case "sethealvalue":
                t(a = "ปรับเลือดตามหน่วย ", e = "message", n = "text", s);
                break;
            case "promote":
                let o = $(this).data("level");
                $.post("https://admin/promote", JSON.stringify({ playerid: id, level: o }));
                break;
            case "giveWeapon":
                i(!1, "#weapons", "#actions");
                break;
            ...
            case "spawnVehicle":
                ...
                break;
            case "inventory":
                $.post("https://admin/inventory", JSON.stringify({ playerid: id }));
                break;
            case "ban":
            ...
            case "setJob":
                ...
                $.post("https://admin/setJob", JSON.stringify({ playerid: id, job: r, rank: l }));
                break;
            case "setTime":
            ...
            case "changeWeather":
                ...
                $.post("https://admin/changeWeather", JSON.stringify({ playerid: id, weather: d }));
                break;
            default:
                $.post("https://admin/" + s, JSON.stringify({ playerid: id }))
        }
    ...
    $("#inner").append(""), document.onkeyup = function(e) { if (27 == e.which) return $.post("https://admin/exit", JSON.stringify({})), void a(!1) }, $("#tpwp-button").click(function() { $.post("https://admin/tp-wp") }), $("header").on("mousedown", function(e) {
