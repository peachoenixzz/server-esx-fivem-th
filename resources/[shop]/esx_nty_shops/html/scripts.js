	...
	if (item.message == "add"){
		$( ".home" ).append('<div class="card">' +
					'<div class="image-holder">' +
						`<img src="nui://nc_inventory/html/img/items/${item.item}.png" onerror="this.src = \'img/default.png\'" alt="' + item.label + '" style="width:100%;">` +
					'</div>' +
					'<div class="container" style=" max-width: 100%;overflow: hidden;text-overflow: ellipsis;">' +
						...
						'<div class="purchase">' +
							...
							'<div class="buy" name="' + item.item + '" data-item-type="'+ item.type +'">Kup</div>' +
						'</div>' +
					'</div>' +
				'</div>');
...
$(".home").on("click", ".buy", function() {
	var $button = $(this);
	var $name = $button.attr('name')
	var type = $button.attr('data-item-type')
