...
MySQL.ready(function ()
    ...
    MySQL.Async.execute('DELETE FROM phone_calls WHERE time  <= DATE_SUB(CURRENT_DATE, INTERVAL 2 DAY)')
    MySQL.Async.execute('DELETE FROM phone_bank WHERE time  <= DATE_SUB(CURRENT_DATE, INTERVAL 2 DAY)')
    MySQL.Async.execute('DELETE FROM phone_app_chat WHERE time  <= DATE_SUB(CURRENT_DATE, INTERVAL 2 DAY)')
    MySQL.Async.execute('DELETE FROM phone_messages   WHERE time  <= DATE_SUB(CURRENT_DATE, INTERVAL 2 DAY)')
    MySQL.Async.execute('DELETE FROM twitter_tweets WHERE time  <= DATE_SUB(CURRENT_DATE, INTERVAL 2 DAY)')
    MySQL.Async.execute('DELETE FROM umpunchmachine_leaderboard')
end)
...
function getPhoneRandomNumber()
    if Config.FourDigit then
        ...
        local numBase = math.random(999999,99999999)
        num = string.format("%05d", numBase )
    end
	return num
end
...
function getSourceFromIdentifier(identifier, cb)
    ...
    for k, user in pairs(xPlayers) do
        --print(user)
        --print("normal iden",identifier)
        --print("xplayer iden",xPlayers.identifier)
...
function getIdentifierByPhoneNumber(phone_number,source)
    ...
    exports.nc_notify:PushNotification(source,{
        ...
        title = 'ระบบโทรศัพท์', -- หัวเรื่อง
        description = 'ไม่มีเบอร์โทรศัพท์นี้ในระบบ', -- รายละเอียด หากต้องการให้แสดงปุ่มกดให้ใช้ tag <btn></btn>
...
function getUserTwitterAccount(source, _identifier)
    ...
    MySQL.Async.fetchAll("SELECT identifier FROM users WHERE identifier = @identifier", {
        ...
        if user == nil then
            ...
            local FirstLastName = "LynnLynn_"..user.identifier
...
function addMessage(source, identifier, phone_number, message)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number,source)
    ...
    local xPlayer = ESX.GetPlayerFromIdentifier(otherIdentifier)
    local mePlayer = ESX.GetPlayerFromIdentifier(identifier)
    local name = otherIdentifier
    if xPlayer then
        name = xPlayer.name
    end
    pcall(function() -- Start log
        exports['azael_dc-serverlogs']:insertData({
            event = 'MessagePhonePost',
            content = ('ผู้เล่น %s ได้ส่งข้อความบนโทรศัพท์ ถึง %s ข้อความ : %s, เบอร์ผู้ส่ง : %s, เบอร์ผู้รับ : %s'):format(mePlayer.name, name, message, myPhone, phone_number),
...
AddEventHandler('gcPhone:internal_startCall', function(source, phone_number, rtcOffer, extraData)
    local sourcePlayer = tonumber(source)
    local srcIdentifier = getPlayerID(sourcePlayer)
    local srcPhone = extraData and extraData.useNumber or getNumberPhone(srcIdentifier)
    ...
    local destPlayer = getIdentifierByPhoneNumber(phone_number,sourcePlayer)
    ...
    AppelsEnCours[indexCall] = callData

    if is_valid then
        --getSourceFromIdentifier(destPlayer, function(srcTo)
        local xPlayer = ESX.GetPlayerFromIdentifier(destPlayer)
            if xPlayer then
                callData.receiver_src = xPlayer.source
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, callData, true)
                TriggerClientEvent('gcPhone:waitingCall', xPlayer.source, callData, false)
                TriggerClientEvent('gcPhone:TgiannSes', -1, xPlayer.source)
            else
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, callData, true)
            end
        --end)
...
RegisterCommand('simcard', function(source) 
