...
function notifyAlertSMS (number, alert, listSrc)
  ...
  if PhoneNumbers[number] == nil then
    ...
    getPhoneNumber(listSrc, function (n)
      if n ~= nil then
        TriggerEvent('gcPhone:_internalAddMessage', alert.numero, n, mess, 0, function (smsMess)
          TriggerClientEvent("gcPhone:receiveMessage", listSrc, smsMess)
        end)
      end
    end)
  end
...
RegisterServerEvent('esx_addons_gcphone:SenderToGang')
AddEventHandler('esx_addons_gcphone:SenderToGang', function (sender, reader, message, coords)
  print(sender, reader, message, coords)
      notifyAlertSMS(reader, {
        message = message,
        coords = coords,
        numero = sender,
      }, source)
