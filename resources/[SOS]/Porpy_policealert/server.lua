...
local function NotifyGang(title, actionType, value, tip, location,streetName,gangName)
    TriggerClientEvent(resourceName .. ":NotifyGang", -1, title, actionType, value, tip, location,streetName,gangName)
end
...
RegisterServerEvent(resourceName .. ":gangAlert")
AddEventHandler(resourceName .. ":gangAlert", function(alertType, sex, streetName, location, gangName)
    local source = source
    local slot = GetAvailableSlot(location)
    ...
    for _, alert in pairs(Config["alert"]) do
        if alertType == alert.type then
            action = alert.action
            alertCooldowns[source] = GetGameTimer() + Config["duration"] * 800
            NotifyGang(action, alert.type, streetName, slot, location, streetName, gangName)
            return
        end
    end
end)
