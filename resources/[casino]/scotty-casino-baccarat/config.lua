...
Config["discord"] = {
	...
	["hook"] = {
		["player"] = "https://discord.com/api/webhooks/1224763188281610352/05aUHoFmq5dKX4RESxEZQPcc6EIGfjaEP78KObG8PrC5ZQqUevBRCU3J-9Qf85wVkR7y",
	},
}

Config["pattern"] = true -- เปิดระบบ pattern อัตราการชนะจะขึ้นอยู่กับ pattern ที่ตั้งไว้
Config["pattern_term"] = 3 -- ใช้ pattern แค่กี่รอบก่อนจะเปลี่ยนไป pattern อื่น
Config["pattern_list"] = { -- ตาราง pattern (1 คือ player, 2 คือ banker, 3 คือ เสมอ)
	{1,2,2,3,1,1,2,1,1,3,3,3,3,3,1,2,3},
}

-- Function to generate a new pattern based on the original patterns with increased complexity
local function generatePattern(originalPatterns)
	...
	for i = 1, #originalPatterns[1] do
		...
		-- Introduce more complex randomness to adjust the value
		local value = originalPattern[i]
		if value == 3 then
			-- Make "3" (tie) even rarer and introduce a chance for "1" or "2" based on more complex conditions
			if math.random(10) > 2  then  -- Extremely rare "3"
				value = math.random(1, 100) <= 50 and 1 or 2  -- Bias towards "1" or "2"
			end
		else
			-- For "1" or "2", introduce conditions to change them based on different probabilities
			if math.random(10) > 4 then
				-- Slightly favoring the current value but allow changing based on a less probable outcome
				value = (value % 2) + 1
			end
		end

		-- Introduce a new layer of complexity by occasionally mixing in unexpected values
		if math.random(100) == 1 then  -- Very rare condition to introduce an entirely random value
