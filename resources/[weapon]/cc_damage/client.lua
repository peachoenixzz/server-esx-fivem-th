...
Config.weaponList = {
    ...
    WEAPON_MACHETTEGOLD = {
        ...
        headshot_dead = false,
        hasEffect = true,
        doEffect = function(isHeadshot, hashWeapon , attacker)
            local ped = PlayerPedId()
            local bonuseffect = math.random(1, 100000)/100000
            print(bonuseffect)
            if bonuseffect <= 0.35 then
                ApplyDamageToPed(ped, math.ceil(33), true)
            end
        end,
        disable_critical_hit = false,
        critical_hit_rate = 1.0,
    ...
    WEAPON_KNUCKLEGOLD = {
        ...
        doEffect = function(isHeadshot, hashWeapon , attacker)
            ...
            if currentArmor >= 18 then
                local bonuseffect = math.random(1, 100000)/100000
                print(bonuseffect)
                if bonuseffect <= 0.35 then
                    local currentHealth = GetEntityHealth(ped)
                    local healthLeft = currentHealth-18
                    SetEntityHealth(ped,healthLeft)
                end
            end
        end,
        disable_critical_hit = false,
    ...
    WEAPON_GUTKNIFELPGOLD = {
        ...
        hasEffect = true,
        doEffect = function(isHeadshot, hashWeapon , attacker)
            ESX.TriggerServerCallback('cc_damge:lifestealattacker', function(status)
                if status then return end
            end,GetPlayerServerId(NetworkGetPlayerIndexFromPed(attacker)))
        end,
        disable_critical_hit = false,
        critical_hit_rate = 1.0,
    },
...
Citizen.CreateThread(function()
    for _,v in pairs(Config.weaponList) do
        ...
        Config.weaponHash[weaponHash] = {
            ...
            critical_hit_rate = v.critical_hit_rate,
        }
...
AddEventHandler('gameEventTriggered', function (name, args)
    ...
    --     local bonuseffect = math.random(1, 100000)/100000
    --     print(bonuseffect)
    --     if bonuseffect <= 0.5 then
    --         local currentHealth = GetEntityHealth(PlayerPedId())
    --         local healthLeft = currentHealth+15
    --         SetEntityHealth(PlayerPedId(),healthLeft)
    --     end
    -- end
        ...
        if not IsEntityDead(args[1]) then
            if HasEntityBeenDamagedByWeapon(args[1], args[7], 0) then
                ...
                if Config.weaponHash[args[7]].hasEffect and args[1] == PlayerPedId() then
                    ...
                    Config.weaponList[Config.weaponHash[args[7]].name].doEffect(isHeadshot, args[7], args[2])
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if GetPedParachuteState(playerPed) == 0 then
            TriggerServerEvent('cc_damage:removePara')
        end
...
RegisterNetEvent('cc_damage:healattacker')
AddEventHandler('cc_damage:healattacker', function()
    local bonuseffect = math.random(1, 100000)/100000
    print(bonuseffect)
    if bonuseffect <= 0.5 then
        local currentHealth = GetEntityHealth(PlayerPedId())
