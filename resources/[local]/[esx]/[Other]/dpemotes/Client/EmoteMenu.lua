...
Citizen.CreateThread(function()
    ...
    while true do
        if Config.FavKeybindEnabled and IsControlPressed(0, Config.FavKeybind) then
            playerPedId = PlayerPedId()
            if not IsPedSittingInAnyVehicle(playerPedId) and FavoriteEmote ~= "" then
                EmoteCommandStart(nil, {FavoriteEmote, 0})
                Wait(3000)  -- Adjust if necessary for optimal response time
            end
        end
        Citizen.Wait(1)  -- Consider increasing if fast response isn't critical
    end
end)
