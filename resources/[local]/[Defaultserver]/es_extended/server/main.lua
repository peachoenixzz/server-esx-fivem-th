...
local loadPlayer = 'SELECT u.accounts, u.job, u.job_grade, u.group, u.position, u.inventory, u.skin, u.loadout, u.metadata, v.priority, u.phone_number'
...
loadPlayer = loadPlayer .. ' FROM users u INNER JOIN vip v ON v.discord_id = u.identifier WHERE u.identifier = ?'
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    for name, item in pairs(ESX.Items) do
      ...
      table.insert(userData.inventory,
        {name = name, count = count, label = item.label, weight = item.weight, usable = Core.UsableItemsCallbacks[name] ~= nil, rare = item.rare,
         canRemove = item.canRemove, limit = item.limit})
    end
  ...
  -- Position
	--print("Position Decode: ",json.decode(result.position))
	--print("Position: ",Config.DefaultSpawn[math.random(1, #Config.DefaultSpawn)])
	if not isNew then
		userData.coords = json.decode(result.position)
	end
	if isNew then
		userData.coords = Config.DefaultSpawn[math.random(1, #Config.DefaultSpawn)] --json.decode(result.position) or
	end

	-- Skin
  if result.skin ~= '' then --result.skin and
  ...
  if result.priority then
    userData.vip = result.priority
    print("Vip name : ", userData.vip)
  end
  ...
  local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job,
    userData.loadout, userData.playerName, userData.coords, userData.metadata, userData.vip, userData.phone_number)
  ...
  xPlayer.triggerEvent('esx:playerLoaded',
    {
	  ...
	  phoneNumber = xPlayer.phoneNumber(),
    }, isNew,
    userData.skin)
...
AddEventHandler('playerOpenQuest', function(source)
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(playerId)

	if xPlayer then
		Core.SavePlayerQuest(xPlayer)
	end
end)

AddEventHandler('playerOpenGachapon', function(source)
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(playerId)

	if xPlayer then
		Core.SavePlayerOpenGachapon(xPlayer)
	end
end)
