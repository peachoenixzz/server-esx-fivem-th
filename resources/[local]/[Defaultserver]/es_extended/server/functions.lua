...
function Core.SavePlayerQuest(xPlayer, cb)
	local parameters <const> = {
		json.encode(xPlayer.getInventory(true)),
		xPlayer.identifier
	}
	...
	MySQL.prepare(
		'UPDATE `users` SET `inventory` = ? WHERE `identifier` = ?',
		parameters,
		function(affectedRows)
			if affectedRows == 1 then
				print(('[^2INFO^7] Saved player For Quest ^5"%s^7"'):format(xPlayer.name))
				TriggerEvent('esx:playerSaved', xPlayer.playerId, xPlayer)
			end
			if cb then
				cb()
...
function Core.SavePlayerOpenGachapon(xPlayer, cb)
	local parameters <const> = {
		json.encode(xPlayer.getInventory(true)),
		xPlayer.identifier
	}
	...
	MySQL.prepare(
		...
		function(affectedRows)
			if affectedRows == 1 then
				print(('[^2INFO^7] Saved player For Open Gachapon ^5"%s^7"'):format(xPlayer.name))
				TriggerEvent('esx:playerSaved', xPlayer.playerId, xPlayer)
			end
			if cb then
				cb()
...
function ESX.GetIdentifier(playerId)
  ...
  for k, v in ipairs(GetPlayerIdentifiers(playerId)) do
    if string.match(v, 'discord:') then
      local identifier = string.gsub(v, 'discord:', '')
      return identifier
    end
  end
end
...
function ESX.UseItem(source, name, ...)
	...
	if item then
		local itemCallback = Core.UsableItemsCallbacks[name]
		...
		if itemCallback then
			...
			if limit ~= -1 and count > limit then
				return pcall(function()
					exports.nc_notify:PushNotification(source, {
						title = 'ไม่สามารถใช้งานสิ่งของชิ้นนี้ได้',
						description = string.format('เนื่องจากคุณมี %s เกินขีดกำจัด', item.label),
						type = 'error',
						duration = 4000
					})
				end)
			...
			local success, result = pcall(itemCallback, source, name, ...)
			if not success then
			return result and print(result) or print(('[^3WARNING^7] An error occured when using item ^5"%s"^7! This was not caused by ESX.'):format(name))
			end
		end
	else
