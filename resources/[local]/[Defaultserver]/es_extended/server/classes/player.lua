...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata, vip, phoneNumber)
	...
	stateBag:set("phoneNumber", self.phoneNumber, true)
	...
	function self.setMoney(money)
		pcall(function() -- Start log
			exports['azael_dc-serverlogs']:insertData({
				event = 'standardSetMoney',
				content = ('Player Name : %s, parse to setMoney(%s), self money before setMoney() is %s'):format(self.getName(), ESX.Math.GroupDigits(money), ESX.Math.GroupDigits(self.getMoney())),
				source = self.source,
			})
		end)
	...
	function self.addMoney(money, reason)
		pcall(function() -- Start log
			exports['azael_dc-serverlogs']:insertData({
				event = 'standardAddMoney',
				content = ('Player Name : %s, parse to addMoney(%s), self money before addMoney() is %s'):format(self.getName(), ESX.Math.GroupDigits(money), ESX.Math.GroupDigits(self.getMoney())),
				source = self.source,
			})
		end)
		money = ESX.Math.Round(money)
	...
	function self.removeMoney(money, reason)
		pcall(function() -- Start log
			exports['azael_dc-serverlogs']:insertData({
				event = 'standardRemoveMoney',
				content = ('Player Name : %s, parse to removeMoney(%s), self money before removeMoney() is %s, '):format(self.getName(), ESX.Math.GroupDigits(money), ESX.Math.GroupDigits(self.getMoney())),
				source = self.source,
			})
		end)
		money = ESX.Math.Round(money)
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if money >= 0 then
			...
			if account then
				...
				pcall(function() -- Start log
					exports['azael_dc-serverlogs']:insertData({
						event = 'standardSetAccountMoney',
						content = ('Player Name : %s, parse to setAccountMoney(%s,%s), self money type %s before setAccountMoney() is %s'):format(self.getName(), accountName, ESX.Math.GroupDigits(money), accountName, self.accounts[account.index].money),
						source = self.source,
					})
				end)
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				local event = 'standardAddAccountMoney'
				local important = false
				if money >= 100000 and money < 1000000 then
					event = 'standardAddAccountMoney100K'
					important = true
				end

				if money >= 1000000 then
					event = 'standardAddAccountMoney1M'
					important = true
				end

				pcall(function() -- Start log
					exports['azael_dc-serverlogs']:insertData({
					event = 'standardAddAccountMoney',
					options = {
						important = important,
					},
					content = ('Player Name : %s, parse to setAccountMoney(%s,%s), self money type %s before setAccountMoney() is %s'):format(self.getName(), accountName, ESX.Math.GroupDigits(money), accountName, self.accounts[account.index].money),
	...
	function self.removeAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				pcall(function() -- Start log
					exports['azael_dc-serverlogs']:insertData({
						event = 'standardRemoveAccountMoney',
						content = ('Player Name : %s, parse to removeAccountMoney(%s,%s), self money type %s before removeAccountMoney() is %s'):format(self.getName(), accountName, ESX.Math.GroupDigits(money), account, self.accounts[account.index].money),
						source = self.source,
					})
				end)
				self.triggerEvent('esx:setAccountMoney', account)
	...
	function self.addInventoryItem(name, count, metadata, slot)
		...
		if item then
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					event = 'standardAddInventoryItem',
					content = ('Player Name : %s, parse to addInventoryItem(%s,%s), self item before addInventoryItem() is %s, '):format(self.getName(), name, count,item.count + count),
					source = self.source,
				})
			end)
	...
	function self.addWeapon(weaponName, ammo)
		if not self.hasWeapon(weaponName) then
			...
			pcall(function() -- Start log
				exports['azael_dc-serverlogs']:insertData({
					event = 'standardAddWeapon',
					content = ('Player Name : %s, parse to addWeapon(%s,%s) '):format(self.getName(), weaponName, ammo),
					source = self.source,
				})
			end)
