-- ตั้งเวลาอยู่ข้างล่างสุด ลงละใช้ได้เลยใครมีตัวเก่าอยู่ลงทับละใช้โล้ด
Config = {
    Locale = 'en-US',
    Distance = { -- All distances in meters
        Draw = 300.0,   -- How close to a plant do you need to be to see it?
    ...
    Items = { -- What items are used?
        Seed = 'Magicseed',     -- Used to plant the weed
        Tend = 'growup',             -- Optional item to progress growth cycle
    ...
    Yield = {3,5}, -- กัญชาที่ได้รับ
    YieldSeed = {1,1}, -- เมล็ดกัญชาที่ได้รับ
    TimeMultiplier = 1.0, -- อัตราการเจริญเติบโต
    Soil = {
...
Config.TimeOfGrowUps = 1
Config.ActionTime = 2 -- เวลาการเติบโตในแต่ละขั้นตอน (เป็นนาที)