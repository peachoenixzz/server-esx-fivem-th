...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            ...
            function(id)
                ...
                TriggerClientEvent('Dv_Hunter_uteknark:planted',-1, id, location, stage)
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ...
                TriggerClientEvent('Dv_Hunter_uteknark:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            ...
            { ['@id'] = id },
            function()
                TriggerClientEvent('Dv_Hunter_uteknark:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('Dv_Hunter_uteknark:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:bulk_data', target, forest)
        else
            TriggerServerEvent('Dv_Hunter_uteknark:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('Dv_Hunter_uteknark:request_data')
    AddEventHandler ('Dv_Hunter_uteknark:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('Dv_Hunter_uteknark:remove')
    AddEventHandler ('Dv_Hunter_uteknark:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:remove', src, plantID)
        end
    end)

    RegisterNetEvent('Dv_Hunter_uteknark:frob')
    AddEventHandler ('Dv_Hunter_uteknark:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('Dv_Hunter_uteknark:bulk_data')
    AddEventHandler ('Dv_Hunter_uteknark:bulk_data', function(forest)
    ...
    RegisterNetEvent('Dv_Hunter_uteknark:planted')
    AddEventHandler ('Dv_Hunter_uteknark:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('Dv_Hunter_uteknark:update')
    AddEventHandler ('Dv_Hunter_uteknark:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('Dv_Hunter_uteknark:removePlant')
    AddEventHandler ('Dv_Hunter_uteknark:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
