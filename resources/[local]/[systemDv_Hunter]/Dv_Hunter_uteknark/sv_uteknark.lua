...
function makeToast(target, subject, message)
    TriggerClientEvent('Dv_Hunter_uteknark:make_toast', target, subject, message)
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('Dv_Hunter_uteknark:do', who, what, where)
end

RegisterNetEvent('Dv_Hunter_uteknark:success_plant')
AddEventHandler ('Dv_Hunter_uteknark:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('Dv_Hunter_uteknark:log')
AddEventHandler ('Dv_Hunter_uteknark:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('Dv_Hunter_uteknark:test_forest')
AddEventHandler ('Dv_Hunter_uteknark:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('Dv_Hunter_uteknark:attempt_plant', source)
        ...
        for id, plant in pairs(cropstate.index) do
            if type(id) == 'number' then -- Because of the whole "hashtable = true" thing
                ...
                if stageData.interact then
                    local relevantTime = plant.data.time + ((growthTime / soilQuality) * 8)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('Dv_Hunter_uteknark:pyromaniac', source)
        end
    end,
}
