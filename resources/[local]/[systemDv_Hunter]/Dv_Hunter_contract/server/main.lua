	...
	if result[1] ~= nil then
		MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
			...
			if rowsChanged ~= 0 then
				...
				Wait(22000)
				TriggerClientEvent('esx:showNotification', _source, 'soldvehicle', plate)
				TriggerClientEvent('esx:showNotification', _target, 'boughtvehicle', plate)
				xPlayer.removeInventoryItem('contract', 1)
			end
		end)
	else
		TriggerClientEvent('esx:showNotification', _source, 'notyourcar')
	end
end)
