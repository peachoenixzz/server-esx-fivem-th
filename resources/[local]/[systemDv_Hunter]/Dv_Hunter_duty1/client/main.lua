    ...
    while true do
        ...
        if CurrentAction ~= nil then
            for _,v in pairs(jobs) do
                if PlayerData.job.name == v or not work then
...
Citizen.CreateThread(function ()
  while true do
    ...
    for _,v in pairs(Config.Zones) do
      if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 12) then
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x) then
        isInMarker  = true
        currentZone = k
        exports.helpnotify:showHelpNotification("กด","E","เพื่อเข้า/ออกเวร")
      end
    end
