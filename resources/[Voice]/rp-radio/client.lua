...
Citizen.CreateThread(function ()
 ...
 PlayerData = ESX.GetPlayerData()
    ...
    if ESX.PlayerData.job and (ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'admin')then
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        -- Open radio settings
        --isActivatorPressed and
        if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead then
            if not exports.esx_policejob:getStatusHandcuffed() then
                if checkHasItem('radio') or checkHasItem('gov_radio') then
                    Radio:Toggle(not Radio.Open)
                end
            end
        elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
        ...
        if Radio.Open then
            ...
            if helpRadio then
                SendNUIMessage({ useRadio = true, currentMhz = radioConfig.Frequency.Current})
                helpRadio = false
            end
            ...
            if not helpRadio then
                helpRadio = true
                SendNUIMessage({ useRadio = false})
            end
